...
def initialise_spectator(spectator_id: str, path_to_spectator_activity: Path,
                         demographics_df: pd.DataFrame) -> Spectator:
    activity = pd.read_csv(path_to_spectator_activity, sep=None, engine="python", dtype={'time': str})
    spectator_demographics = demographics_df.loc[demographics_df['﻿GENEActiv ID'] == spectator_id, :]
    return Spectator(
        spectator_id,
        ...
        spectator_demographics["How important to YOU is it that YOUR team wins?"].iat[0],
        spectator_demographics["How strongly do YOU see yourself as a fan of YOUR team?"].iat[0],
...
def initialise_spectators(path_to_activity: Path, path_to_demographics: Path) -> List[Spectator]:
    demographics = pd.read_csv(path_to_demographics, sep=None, engine="python", dtype={'﻿GENEActiv ID': str})
    spectators = []
    for file in [file for file in os.listdir(path_to_activity) if file.find(".csv")!=-1]:
        spectators.append(initialise_spectator(extract_id(file), path_to_activity/file, demographics))
    return spectators
