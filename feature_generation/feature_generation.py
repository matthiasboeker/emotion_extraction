...
def get_train_test_split(feature_df: pd.DataFrame, test_size: float, simple_labels: bool,
    ...
    #train_ids, test_ids = train_test_split(list(set(feature_df["id"])), test_size=test_size, **kwargs)
    #train_df = feature_df.loc[feature_df["id"].isin(train_ids)]
    #test_df = feature_df.loc[feature_df["id"].isin(test_ids)]
    output_df = feature_df.drop(["control", "goal"], axis=1)
    ...
    #return train_df.drop(["control", "id", "goal"], axis=1), \
    #       test_df.drop(["control", "id", "goal"], axis=1), \
    #       train_df["control"], \
    #       test_df["control"]
    #train_test_split(output_df, labels, test_size=test_size, **kwargs)
