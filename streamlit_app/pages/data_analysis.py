...
def data_analysis(spectators, match, ssa_objs):
    ...
    ts_df = pd.DataFrame(
        ...
        columns=[spectator.id for spectator in spectators],
    )
    ...
    if show_analysis == "Show correlation matrix":
        ...
        sns.heatmap(
            ...
            ax=ax,
            annot=True,
            annot_kws={"fontsize": 6, "fontweight": "normal", "fontfamily": "serif"},
        )
        st.pyplot(fig_corr)
    if show_analysis == "Show distributions":
        ...
        for ticker, ax in zip(ts_df.columns, axs.ravel()):
            ax.hist(
                ts_df[str(ticker)].rolling(60).mean()[60:],
                bins=20,
                density=True,
                edgecolor="black",
                linewidth=0.5,
    ...
    if show_analysis == "Decompose time series":
        ...
        fig_decomp, (ax_trend, ax_season, ax_resid) = plt.subplots(
            3, 1, figsize=(10, 7)
        )
